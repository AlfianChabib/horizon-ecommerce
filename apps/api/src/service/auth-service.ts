...
import { toSessionResponse } from '../model/user-model';
...
export class AuthService {
  ...
  static async login(payload: LoginPayload): Promise<ResponseLogin> {
    ...
    const user = await prisma.user.findUnique({
      ...
      include: { authInfo: { include: { socialAuth: true, localAuth: true } }, profile: true },
    });
    ...
    const userData = toSessionResponse(user as User & { profile: Profile });
  ...
  static async getSession(userId: number) {
    ...
    if (!user || !user.profile) throw new ResponseError(400, 'User not found');

    const data = toSessionResponse(user as User & { profile: Profile });
    return data;
  }
